...
fn prepare_tensorflow_source() -> PathBuf {
    ...
    let download_absl_dir = tf_src_dir.join("lite/tools/make/downloads/absl");
    if !download_absl_dir.exists() {
        fs_extra::dir::copy(
...
fn prepare_tensorflow_library() {
    ...
    #[cfg(not(feature = "build"))]
    {
        ...
        // let static_dynamic = if Path::new(&lib_dir).join("libtensorflowlite_flex.a").exists() {
        //     "static"
        // } else {
        //     "dylib"
        // };
...
fn import_tflite_types() {
    ...
    let bindings = Builder::default()
        ...
        .clang_arg(format!("-I{}/deps/flatbuffers/include", submodules_str))
...
fn build_inline_cpp() {
    ...
    cpp_build::Config::new()
        ...
        .include(submodules.join("deps/flatbuffers/include"))
        .flag("-fPIC")
        .flag("-std=c++17")
